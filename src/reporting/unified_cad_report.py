...
def build_unified_report(result: Any) -> bytes:
    ...
    # ── Cover / title ─────────────────────────────────────────────────────────
    has_comparison = bool(result.comparison_pages)
    ...
    if has_comparison:
        story.append(Paragraph(f"Original: {_escape(result.original_name)}", body))
    story.extend([
    ...
    # ── 4. Difference Map with IDs (only in comparison mode) ────────────────
    if has_comparison:
