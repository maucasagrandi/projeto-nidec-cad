    ...
    *,
    ...
    for gt in gt_list:
        ...
        if matched:
            ...
            # Borderline IoU pairs are accepted because the manual ROI overlap
            # criterion confirms that both boxes cover the same feature. Keep
            # that provenance visible instead of reporting the match as a
            # strong IoU match merely because it crossed the minimum narrowly.
            match_reason = (
                "manual_roi_overlap"
                if overlap_match and best_iou < 0.50
                else "iou"
            )
        else:
            fn += 1
